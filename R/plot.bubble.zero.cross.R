plot.bubble.zero.cross <- function (xyzRaw, group = rep("A", nrow(xyzRaw)), Groups = unique(group), Circle.Area.Prop.to.Z = TRUE, maxsize = scale.size * diff(range(xyzRaw[, 2], na.rm = TRUE)), 
    ...
    fill.col = colorRampPalette(colors = c("dodgerblue", "orange", "cyan", "magenta", "green", "red", "blue", "violetred4", "cyan3", "mediumorchid3", "seagreen4", "black"))(N), fill.col.alpha = 0.2, 
    ...
    if (legend) {
        ...
        if(cross.cex > 0) {
            text(Usr[1] + legLoc[1] * (Usr[2] - Usr[1]), Usr[3] + (legLoc[2] - 0.03) * (Usr[4] - Usr[3]), "+", col = cross.col, cex = cross.cex * 3)
